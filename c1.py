...
def ship_name(fleet, designated_no):
    ...
    from the fleet."""
    return fleet.get(designated_no, "does not exist")

def how_many_5(numbers, treshold = 5):
    """Returns number of numbers greater than 5."""
    return sum(1 for i in numbers if i > treshold)

def gen_list_gt(lst, no):
    """Returns list with numbers greater than no."""
    return [i for i in lst if i > no]
